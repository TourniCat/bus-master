...
var route_colors: Array[Color] = [Color("#ef6a73"), Color("#5f8fd8"), Color("#62a96b")]
var routes: Array[Dictionary] = []
var active_route: int = 0
var buses: Array[Dictionary] = []

var sim_running: bool = false
var sim_minutes: float = 6.0 * 60.0
var day: int = 1
var spawn_accumulator: float = 0.0
var delivered: int = 0
var total_spawned: int = 0
var last_logged_hour: int = -1
var status_text: String = "Build a route: choose R1/R2/R3, then click stops. Add buses and press Start."
...
func _ready() -> void:
	...
	_trace("START build=%s godot=%s" % [PROTOTYPE_BUILD, String(Engine.get_version_info().get("string", "unknown"))])
	queue_redraw()

func _trace(message: String) -> void:
	print("[BUS_MASTER] %s" % message)

func _stop_node(stop_index: int) -> int:
	return int(stops[stop_index]["node"])
...
func _log_hourly_state() -> void:
	var current_hour: int = int(sim_minutes / 60.0) % 24
	if current_hour == last_logged_hour:
		return
	last_logged_hour = current_hour
	_trace("SIM day=%d time=%02d:00 waiting=%d delivered=%d fleet=%d rating=%d" % [
		day, current_hour, _total_waiting(), delivered, _total_bus_count(), _rating()
	])
...
func _handle_pointer(pos: Vector2) -> void:
	var rects: Array = _button_rects()
	for i in range(3):
		var route_button: Rect2 = rects[i]
		if route_button.has_point(pos):
			_select_route(i)
			return
	var minus_button: Rect2 = rects[3]
	var plus_button: Rect2 = rects[4]
	var start_button: Rect2 = rects[5]
	var reset_button: Rect2 = rects[6]
	if minus_button.has_point(pos):
		_change_bus_count(-1)
		return
	if plus_button.has_point(pos):
		_change_bus_count(1)
		return
	if start_button.has_point(pos):
		_toggle_simulation()
		return
	if reset_button.has_point(pos):
		_reset_prototype()
		return

	var nearest: int = -1
	var nearest_dist: float = 24.0
	for i in range(stops.size()):
		var p: Vector2 = road_points[_stop_node(i)]
		var distance: float = pos.distance_to(p)
...
func _edit_active_route(stop_index: int) -> void:
	...
	if route_stops.size() > 0 and int(route_stops[-1]) == stop_index:
		...
		status_text = "Added %s to Route %d" % [String(stops[stop_index]["name"]), active_route + 1]
		_trace("ACTION route=%d add_stop=%s" % [active_route + 1, String(stops[stop_index]["name"])])
	routes[active_route]["stops"] = route_stops
	_respawn_route_buses(active_route)

func _change_bus_count(delta: int) -> void:
	var current: int = int(routes[active_route]["bus_count"])
	if delta > 0:
		var route_stops: Array = routes[active_route]["stops"]
	...
	_trace("ACTION route=%d buses=%d total_fleet=%d" % [active_route + 1, current, _total_bus_count()])

func _total_bus_count() -> int:
	var result: int = 0
	for route in routes:
		result += int(route["bus_count"])
	return result
...
func _toggle_simulation() -> void:
	...
	_trace("ACTION simulation=%s" % ("running" if sim_running else "paused"))
...
func _spawn_passengers() -> void:
	var hour: float = sim_minutes / 60.0
	var pairs: Array = []
	var amount: int = 1
	...
	for _n in range(amount):
		var pair_index: int = int(randi() % pairs.size())
		var pair: Array = pairs[pair_index]
		_add_waiting_passenger(int(pair[0]), int(pair[1]))
...
func _respawn_route_buses(route_index: int) -> void:
	for i in range(buses.size() - 1, -1, -1):
		if int(buses[i]["route"]) == route_index:
			buses.remove_at(i)
	var route_stops: Array = routes[route_index]["stops"]
	var count: int = int(routes[route_index]["bus_count"])
	if route_stops.size() < 2:
		return
	for i in range(count):
		var divisor: float = maxf(1.0, float(count))
		var start_slot: int = int(floor(float(i) * float(route_stops.size()) / divisor))
		start_slot = clampi(start_slot, 0, route_stops.size() - 1)
		var direction: int = 1 if i % 2 == 0 else -1
		if start_slot == 0:
			direction = 1
		elif start_slot == route_stops.size() - 1:
			direction = -1
		var start_stop_index: int = int(route_stops[start_slot])
		var bus: Dictionary = {
			"route": route_index,
			"slot": start_slot,
			"direction": direction,
			"pos": road_points[_stop_node(start_stop_index)],
			"path": [],
			"path_cursor": 1,
...
func _setup_next_leg(bus: Dictionary) -> void:
	var route_index: int = int(bus["route"])
	var route_stops: Array = routes[route_index]["stops"]
	if route_stops.size() < 2:
		bus["path"] = []
		return
	var current_slot: int = int(bus["slot"])
	var direction: int = int(bus["direction"])
	var next_slot: int = current_slot + direction
	...
	var from_stop_index: int = int(route_stops[current_slot])
	var to_stop_index: int = int(route_stops[next_slot])
	var from_node: int = _stop_node(from_stop_index)
	var to_node: int = _stop_node(to_stop_index)
	var node_path: Array = _shortest_path_nodes(from_node, to_node)
	var positions: Array[Vector2] = []
	for node_value in node_path:
		positions.append(road_points[int(node_value)])
	bus["path"] = positions
	bus["path_cursor"] = 1
...
func _shortest_path_nodes(start_node: int, end_node: int) -> Array:
	...
	while not unvisited.is_empty():
		...
		for i in range(1, unvisited.size()):
			var candidate: int = int(unvisited[i])
		...
		for neighbor_value in neighbors:
			...
			if not unvisited.has(neighbor):
				continue
			var alt: float = float(dist[current]) + road_points[current].distance_to(road_points[neighbor])
			if alt < float(dist[neighbor]):
				dist[neighbor] = alt
				prev[neighbor] = current
	var path: Array = []
	var cursor: int = end_node
	while cursor != -1:
		path.push_front(cursor)
		if cursor == start_node:
			break
		cursor = int(prev[cursor])
	if path.is_empty() or int(path[0]) != start_node:
...
func _waiting_at_stop(stop_index: int) -> int:
	var total: int = 0
	var waiting: Dictionary = stops[stop_index]["waiting"]
	for count_value in waiting.values():
...
func _rating() -> int:
	return clampi(100 - int(float(_total_waiting()) * 0.55), 0, 100)
...
func _draw_background() -> void:
	...
	for edge in road_edges:
		var a: Vector2 = road_points[int(edge[0])]
		var b: Vector2 = road_points[int(edge[1])]
...
func _draw_routes() -> void:
	for route_index in range(routes.size()):
		...
		for i in range(route_stops.size() - 1):
			var from_stop_index: int = int(route_stops[i])
			var to_stop_index: int = int(route_stops[i + 1])
			var node_path: Array = _shortest_path_nodes(_stop_node(from_stop_index), _stop_node(to_stop_index))
			var points := PackedVector2Array()
			for node_value in node_path:
				points.append(road_points[int(node_value)])
			if points.size() >= 2:
...
func _draw_buses() -> void:
	for bus in buses:
		var pos: Vector2 = bus["pos"]
		var route_index: int = int(bus["route"])
		var color: Color = route_colors[route_index]
		...
		var onboard: Array = bus["onboard"]
		var load_text: String = "%d" % onboard.size()
		draw_string(font, pos + Vector2(-5, -9), load_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color("#343434"))

func _draw_stops() -> void:
	for i in range(stops.size()):
		var pos: Vector2 = road_points[_stop_node(i)]
		var waiting: int = _waiting_at_stop(i)
		if waiting > 0:
			var halo_radius: float = 17.0 + minf(18.0, float(waiting) * 0.35)
			draw_circle(pos, halo_radius, Color(0.88, 0.32, 0.30, 0.16))
		draw_circle(pos, 11.0, Color("#fffdf7"))
		draw_arc(pos, 11.0, 0.0, TAU, 32, Color("#383838"), 2.0, true)
		draw_string(font, pos + Vector2(-30, -18), String(stops[i]["name"]), HORIZONTAL_ALIGNMENT_CENTER, 60, 12, Color("#343434"))
		if waiting > 0:
...
func _draw_hud() -> void:
	...
	var hour: int = int(sim_minutes / 60.0) % 24
	var minute: int = int(sim_minutes) % 60
	var time_text: String = "Day %d   %02d:%02d" % [day, hour, minute]
	...
	for i in range(3):
		...
		var fill: Color = route_colors[i] if active_route == i else Color("#e4e0d8")
		draw_rect(rect, fill, true)
		draw_rect(rect, Color("#595959"), false, 1.5)
		draw_string(font, rect.position + Vector2(12, 26), "Route %d" % (i + 1), HORIZONTAL_ALIGNMENT_LEFT, -1, 15, Color("#303030"))
	...
	var active_stops: Array = routes[active_route]["stops"]
	var route_info: String = "R%d: %d stops / %d buses" % [active_route + 1, active_stops.size(), int(routes[active_route]["bus_count"])]
